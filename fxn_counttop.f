...
*************************OVERALL COMMONS *****************************
ctesttop
      common/testtopi/itoptest
      common/testtopt/rinvmassa
...
*******************PHASE SPACE and MULTICHANNEL **********************
      DIMENSION p(0:3,8),pcoll(0:3,8)
c giuseppe 29/09/2007
      INTEGER iphsp
c end giuseppe 29/09/2007
*************************PERMUTATIONS ********************************
      INTEGER idin0(2),ipos0(3,-16:21),index(3,6),ifact(0:3),
...
*     CALL AMPLITUDE: USE CM MOMENTA
      ...
      DO i=1,8
        DO mu=0,3
          p(mu,i)=p(mu,i)*idp_inout(i)
        ENDDO                   !mu
      ENDDO                     !i
ctot
*sandro2/3/07
cc giuseppe 26/02/2007
cc      if (i_nofullew.eq.-1.)then
c      if (i_nofullew.eq.-1)then
cc end giuseppe 26/02/2007
      if (i_pertorder.eq.1)then
*sandro2/3/07end
        CALL amp(p,idp,igr,igo,ionesh,symfact,amod,iordamp)
        amod=amod*elcharge2**6
      else
ctotend
        IF(ngluon.EQ.0)THEN
          CALL ampqcd(p,idp,igr,igo,ionesh,symfact,amod,iordamp)
        ELSEIF(ngluon.EQ.2)THEN
          CALL amp2g(p,idp,igr(5),igo(1,5),ionesh,symfact,ngin,amod,
     &       iordamp)
        ENDIF
ctot
      endif
...
*old*          ENDIF
          IF(iwrite_event.EQ.1)THEN
            WRITE(23,'(A)')'<event>'
            WRITE(23,'(2(I4,1x),4(E13.6,1x))')
     &              NUP,IDPRUP,XWGTUP,SCALUP,AQEDUP,AQCDUP
ctoptest
            write(23,*) 'itoptest,rinvmassa', itoptest,rinvmassa
ctoptestend
            DO i=1,NUP
              WRITE(23,'(6(I4,1x),5(E13.6,1x),2(E10.1))')
