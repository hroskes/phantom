...
* just once, independently on the process.
      ...
      IF(rmh.LT.150.d0)THEN
        xhiggswidth=30.d0+(rmh-120.d0)/(150.d0-120.d0)*(20.d0-30.d0)
      ELSEIF(rmh.LT.250.d0)THEN
        xhiggswidth=20.d0+(rmh-150.d0)/(250.d0-150.d0)*(3.d0-20.d0)
      ELSEIF(rmh.LT.1.d3)THEN
        xhiggswidth=3.d0+(rmh-250.d0)/(1.d3-250.d0)*(1.d0-3.d0)
      ENDIF
      IF(rmh.LT.1.d3.AND.rmh.GE.0.d0)THEN
        spmhlo=rmh-xhiggswidth*gamh
        spmhhi=rmh+xhiggswidth*gamh
      ELSE
* IF rmh > 1.d3 GeV or rmh < 0 the Higgs resonance is not mapped
...
* Heavy Higgs
      IF(rmhh.LT.150.d0)THEN
        xhhiggswidth=30.d0+(rmhh-120.d0)/(150.d0-120.d0)*(20.d0-30.d0)
      ELSEIF(rmhh.LT.250.d0)THEN
        xhhiggswidth=20.d0+(rmhh-150.d0)/(250.d0-150.d0)*(3.d0-20.d0)
      ELSEIF(rmhh.LT.1.d3)THEN
        xhhiggswidth=3.d0+(rmhh-250.d0)/(1.d3-250.d0)*(1.d0-3.d0)
      ENDIF
      IF(rmhh.LT.1.d3.AND.rmhh.GE.0.d0)THEN
        spmhhlo=rmhh-xhhiggswidth*gamhh
        spmhhhi=rmhh+xhhiggswidth*gamhh
      ELSE
* IF rmhh > 1.d3 GeV or rmhh < 0 the higgs resonance is not mapped
* When both a Z and a higgs resonance are possible (n5 > 0) only the
* first is activated
        spmhhlo=ecoll
        spmhhhi=ecoll
      ...
      IF(spmzhi.GE.spmhlo .OR. spmhhi.GE.spmhhlo)THEN
      WRITE(*,*)
     & '***************************************************************'
      WRITE(*,*)
     & '                          WARNING                              '
